...
def eval_estimator(m, eigfuh,eigv):  
    ...
    grad_basis = basis.with_element(ElementVector(ElementDG(ElementTriP0()))) 
...
def eval_estimator_adjoint(m, eigfuh,eigv):  
    ...
    def interior_residual_vh(w): 
        ...
        x, y = w.x
        (wxx, wxy), (wyx, wyy) = w['grad_w'].grad 
        return h ** 2 * abs(uh + wxx + wyy + 2*(w['grad_w'][0]+w['grad_w'][1])) ** 2 
...
for itr in reversed(range(itrMax)):
    ...
    if itr > 0:
        ...
        from skfem.visuals.matplotlib import *
        draw(m)
        plt.savefig('Kellogg_P1_theta075_{}.pdf'.format(itrMax - itr),bbox_inches='tight')
        plt.close() 
